// frontend/src/pages/admin/siniestros/HistorialConductor.jsx
...
import { 
  ...
  TrendingUp, AlertCircle, Phone, Mail
} from 'lucide-react';
...
const HistorialConductor = () => {
  ...
  const cargarDatos = async () => {
    try {
      ...
      const conductorResponse = await adminService.getConductorById(id);
      console.log('Conductor:', conductorResponse);
      setConductor(conductorResponse.conductor || conductorResponse);
      
      // Cargar historial de siniestros
      const siniestrosResponse = await adminService.getSiniestrosConductor(id);
  ...
  const exportarPDF = () => {
    ...
    doc.text('Historial de Siniestros por Conductor', 14, 20);
    
    // Información del conductor
    doc.setFontSize(12);
    doc.setTextColor(0, 0, 0);
    doc.text(`Conductor: ${conductor?.nombre_conductor || 'N/A'}`, 14, 30);
    doc.text(`Teléfono: ${conductor?.numero_telefono || 'N/A'}`, 14, 37);
    doc.text(`Email: ${conductor?.email || 'N/A'}`, 14, 44);
    ...
    autoTable(doc, {
      startY: 75,
      head: [['Folio', 'Fecha', 'Tipo', 'Vehículo', 'Gravedad', 'Estado', 'Costo']],
      body: tableData,
      styles: { fontSize: 8 },
      headStyles: { fillColor: [0, 128, 255] }
    });
    
    doc.save(`Historial_Siniestros_${conductor?.nombre_conductor?.replace(/\s+/g, '_') || 'conductor'}_${new Date().toISOString().split('T')[0]}.pdf`);
  };
      ...
      <div className="glass rounded-lg p-6 border border-primary/20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            ...
            <p className="text-white font-semibold">{conductor?.nombre_conductor || 'N/A'}</p>
          </div>
          <div>
            <p className="text-gray-400 text-sm">Teléfono</p>
            <div className="flex items-center gap-2">
              <Phone className="w-4 h-4 text-gray-400" />
              <p className="text-white font-semibold">{conductor?.numero_telefono || 'N/A'}</p>
            </div>
          </div>
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <p className="text-white font-semibold truncate">{conductor?.email || 'N/A'}</p>
            </div>
          </div>
          <div>
            <p className="text-gray-400 text-sm">Estado</p>
            <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${
              conductor?.status === 'Aprobado' ? 'bg-green-500/20 text-green-400' :
              conductor?.status === 'Pendiente' ? 'bg-yellow-500/20 text-yellow-400' :
              'bg-red-500/20 text-red-400'
            }`}>
              <div className="w-2 h-2 rounded-full bg-current"></div>
              {conductor?.status || 'N/A'}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="glass rounded-lg border border-primary/20">
        ...
        {siniestros.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <p className="text-gray-400">Este conductor no tiene siniestros registrados</p>
          </div>
        ) : (
